...
<body>
	...
	<script>
		...
		function ImageCheckbtn(){
			var img = $('#pic')[0].src
			
			if(img=="") {
				alert.log("분석하실 사진을 업로드 하세요");
		...
		<!-- 여기서 쓰이는 매개변수 pic은
		1. 분석하기 버튼을 눌렀을 때 pic이 존재할 시 아래 병충해 설명, 병충해 해결방안 칸이 나타나게 하는 함수
		1.1. pic이 존재하지 않는다면, 분석할 사진을 업로드 하세요 출력
		2. 장고로 보내져서 .weight파일로 모델 분석을 위한 것 
		3. 데이터베이스로 전송-->
		<!-- 
		1. onclick 함수에서 tag안에 값 출력 => display:none
		2. pic을 매개변수로 인식하기
		-->
	...
	<main>
		<div class="container px-4 px-lg-5">
				...
				<!-- 웹상에서 파일 선택 부분 -->
					<form action="/saveImage" enctype="multipart/form-data" method="post">
						...
						<input type="file" id="camera" name="camera" capture="camera"
							accept="image/*" /> <br/> <img id="pic" style="width:256px; height:256px"/>
			...
			<button id="button1" onclick="ImageCheckbtn('abababab')">분석하기</button>
			<button id="button1" onclick="alert('dfdfdf')">분석하기 테스트1</button>
			<button id="button1" onclick="platttttt()">분석하기 테스트2</button>
			<!-- 여기서 쓰이는 매개변수 pic은
			1. pic이 존재할 시 아래 병충해 설명, 병충해 해결방안 칸이 나타나게 하는 함수 
			2. 데이터베이스로 전송 
			3. 장고로 보내져서 .weight파일로 모델 분석을 위한 것 -->
